...
@section('content')
    ...
    <section>
        ...
        <div class="section-one-content single-news container mx-auto">
            <div class="news_body_main symbol_content">
                <div class="single-news-headtext">
                    <h1>{{ $symbol->title }}</h1> <br>
                </div>
                <div class="symbol_img_card">
                    @php $images = json_decode($symbol->image) @endphp
...
{{--                            <span class="flex items-center gap-x-2"><span class="iconify" data-icon="mdi:clock-time-four-outline" data-inline="false"></span> {{ \Carbon\Carbon::parse($symbol->created_at)->format('H:m / d.m.Y') }}</span>--}}
