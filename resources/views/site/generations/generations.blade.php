          ...
          @foreach($generations as $generation)
            <div class="category-news-content-main">
                ...
                <div class="category-news-date">
                    <span class="flex items-center"><span class="iconify" data-icon="mdi:clock-time-four-outline" data-inline="false"></span> {{ \Carbon\Carbon::parse($generation->created_at)->format('H:m / d.m.Y') }}</span>
                </div>
                <a href="{{ route('generation', ['id' => $generation->id]) }}">{{ $generation->fio }}</a>
            </div>
          @endforeach
